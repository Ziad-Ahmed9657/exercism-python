...
def exchange_money(budget, exchange_rate):
    ...
    total_money = float(f"{budget / exchange_rate:.2f}")
...
def get_change(budget, exchanging_value):
    ...
    Money_Left = float(f"{budget - exchanging_value:.2f}")
    return Money_Left
...
def get_leftover_of_bills(amount, denomination):
    Left_Over_Of_Bills = float(f"{amount % denomination:.2F}")
    return Left_Over_Of_Bills
...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    Exchange_Budget = budget // New_Exchange_Rate

    Number_Of_Bills = Exchange_Budget // denomination
